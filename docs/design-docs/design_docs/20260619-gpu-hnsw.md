...
> **⚠️ Key limitation — filtered search is not supported.** A GPU_HNSW / GPU_HNSW_SQ
> segment does **not** apply the `BitsetView` that Milvus uses for deletes, TTL
> expiry, and partition/visibility filters; Knowhere `Search()` returns
> `invalid_args` when a non-empty bitset is present (the CPU index copy is freed
> after GPU upload, so there is no in-node CPU fallback). Because sealed segments
> on any collection with deletes routinely carry a non-empty bitset, **GPU_HNSW is
> suitable only for append-mostly / immutable collections.** GPU-side bitset
> filtering is a tracked follow-up. See the [Failure Modes](#failure-modes) table
> and the [OCQ-removal / limitations decision record](20260711-gpu-hnsw-ocq-removal.md).
...
| Filtered search (delete / TTL / partition bitset) | **Not supported.** A GPU_HNSW/GPU_HNSW_SQ segment does not apply bitsets, and its CPU index copy is freed after GPU upload, so an in-node CPU fallback is not available. Knowhere `Search()` returns `invalid_args` when a non-empty bitset is present. GPU_HNSW is therefore suitable only for append-mostly / immutable collections that do not accumulate deletes, TTL expiry, or partition filters. See the OCQ-removal / limitations decision record for the follow-up (GPU-side bitset filtering). |
